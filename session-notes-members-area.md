...
**Security fix (found in final review):** the coach API routes had **no server-side auth**. `src/proxy.ts` (Next 16's renamed middleware) only gated `/coach/*` *pages* — `pathname.startsWith('/coach')` never matches `/api/coach/*` or `/api/inbody`. So `GET /api/coach/strength` / `GET /api/inbody` (which return recent rows *with ids*) plus every coach `POST`/`PATCH`/`DELETE` on those paths were anonymously reachable — trivial to enumerate and wipe other members' records via the service-role client. Fixed in `proxy.ts`: added `const isCoach = user?.user_metadata?.role === 'coach'` and a check that returns `401` JSON for `pathname.startsWith('/api/coach/') || pathname === '/api/inbody'`. Verified in-browser: those paths now 401 without a coach session; `/api/inbody/member` (member route, GymMaster-cookie auth) is unaffected. All `/api/coach/*` callers are coach-portal contexts; member messaging uses `/api/messages`, not `/api/coach/messages`, so nothing member-facing broke. **Not done:** other pre-existing coach routes (`/api/coach/awards`, `/challenges`, `/messages`, `/content/*`, `/members/*/tasks`) are now covered by the same `startsWith('/api/coach/')` guard as a side benefit, but were never individually audited.

Other review follow-ups applied (`17520a8`): member delete handlers `throw` on non-ok so `InlineDeleteConfirm` resets instead of the row silently reappearing; strength history list capped `max-h-44 overflow-y-auto`; `PATCH /api/coach/strength` returns a generic message instead of raw `error.message`. Not addressed (minor / pre-existing): modals have no focus-trap / scroll-lock / dirty-check on backdrop click; `PATCH` on a missing id returns 200; `formatDateLabel` duplicated between `ScanHistoryTable` and `body-composition/page.tsx`; strength card shows both the mini-strip and the editable list in the default view.

**Verified:** `npx tsc --noEmit` clean; `npm run build` clean. Browser: member strength delete round-trips end to end (single-result card shows the list, bin → confirm → row + DB row gone); coach API paths 401 without a session. InBody member delete + the coach edit/delete UI flows are code/type/build-verified only (coach portal needs a login not available in this session). Pre-existing unused-var warning `maxWeight` in `body-composition/page.tsx` left untouched (not introduced here).
